...
# 按秒 视频截取 截取 duration 时长的视频 从 str_second 秒开始截取
def v_intercept_sec(input_file, str_second, duration, out_file):
    try:
        cmd = 'ffmpeg -y -i "{}" -ss {} -t {} -f mp4 {}'.format(input_file, str_second, duration, out_file)
        res = subprocess.call(cmd, shell=True)
...
# 按时间 视频截取 从 str_start_time 开始截取，到 str_end_time
def v_intercept_time(input_file, str_start_time, str_end_time, out_file):
    try:
        cmd = 'ffmpeg -ss {} -to {} -i "{}" -c copy "{}"'.format(str_start_time, str_end_time, input_file, out_file)
        res = subprocess.call(cmd, shell=True)
