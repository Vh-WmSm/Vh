...
class line_chart:
    def init(self):
        ...
        button_name = '开始'
        default_value_lis = ['folder',
                             ['各个设备生成各自折线图', '各个设备折线图整合取绝对误差', '前两个选项都执行'],
                             desktop_path]
        GUI = public_tools.PySimpleGUI_Tool(title_name_lis, windows_name, button_name, default_value_lis, in_size=25)
        ...
        for li in lis:
            ...
            if suffix in ('xls', 'xlsx'):
                ...
                if self.mode == '各个设备生成各自折线图':
                    self.mode_key = '1'
                    self.single_output_file_name = self.during_time + '设备数据.xlsx'
                    if self.single_output_file_name in os.listdir(self.target_path):
                        self.single_integration_wb = openpyxl.load_workbook(self.target_path + '\\' + self.single_output_file_name)
                        self.single_integration_ws = self.single_integration_wb.create_sheet(self.ws['a2'].value, self.xls_count - 1)
                    else:
                        self.single_integration_wb = openpyxl.Workbook()
                        self.single_integration_ws = self.single_integration_wb.active
                        self.single_integration_ws.title = self.ws['a2'].value
                    self.single_mapping()
    ...
    def single_mapping(self):
        ...
        self.single_integration_ws.column_dimensions['a'].width = 20  # 联网设备列
        self.single_integration_ws.column_dimensions['e'].width = 15  # 从机名称列
        self.single_integration_ws.column_dimensions['f'].width = 17  # 时间列
        # 遍历该表，写入整合表
        for value in self.ws.iter_rows(min_row=1, max_row=max_row, min_col=1, max_col=max_column, values_only=True):
            value = list(value)
            self.single_integration_ws.append(value)
        self.wb.close()
        # 所有数据居中
        for i in range(1, max_row + 1):
            for j in range(1, max_column + 1):
                self.single_integration_ws.cell(i, j).alignment = Alignment(horizontal='center', vertical='center')
        # 画折线图，定义LineChart对象
        chart = LineChart()
        # 定义x坐标的数据
        x_data = Reference(self.single_integration_ws, min_col=6, min_row=2, max_col=6, max_row=max_row)
        # 定义作图数据列
        data = Reference(self.single_integration_ws, min_col=7, min_row=1, max_col=7, max_row=max_row)
        ...
        chart.height = 50
        chart.width = 150
        self.single_integration_ws.add_chart(chart)
        self.single_integration_wb.save(self.target_path + '\\' +self.single_output_file_name)
        self.single_integration_wb.close()
    ...
    def table_integration(self):
        ...
        if self.integration_ws['a1'].value is None:
            self.integration_ws['a1'] = f'{self.ws["a2"].value}'
    ...
    def integration_mapping(self):
        ...
        # 定义作图数据列
        max_col = 0
        for i in range(self.integration_max_column):
            if '|' not in self.integration_ws[f'{chr(65 + i)}1'].value:
                max_col += 1
        data = Reference(self.integration_ws, min_col=2, min_row=1, max_col=max_col, max_row=self.integration_max_row)
        # 添加数据到chart（from_rows=False说明是以列数据来画图，titles_from_data=False说明不画图例，若为True则以第一行文字写图例，这时上方的data处应min_row=1）
        chart.add_data(data, from_rows=False, titles_from_data=True)
        # 添加x坐标数据
        chart.set_categories(x_data)
        ...
        chart = LineChart()
        x_data = Reference(self.integration_ws, min_col=1, min_row=2, max_col=1, max_row=self.integration_max_row)
        data = Reference(self.integration_ws, min_col=self.tem_integration_max_column + 1, min_row=1,
                         max_col=self.integration_max_column, max_row=self.integration_max_row)
        chart.add_data(data, from_rows=False, titles_from_data=True)
        chart.set_categories(x_data)
    ...
    def time_disposal(self, time):
        ...
        # 分
        min = time_lis[1]
        hour_add = self.time_add // 60
        min_add = self.time_add % 60
        # 分钟数延迟问题处理
        min = int(min) + min_add
        hour = int(hour) + hour_add
        if min > 59:
            min -= 60
            hour = int(hour) + 1
        if hour > 23:
